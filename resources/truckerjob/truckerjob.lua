...
local Truck = { "PACKER", "PHANTOM" }
local Trailer = { "TANKER", "TRAILERS", "TRAILERS2", "TRAILERLOGS" }

local MissionData = {
    [0] = { 594.761291503906, -2770.05224609375, 6.05681467056274, 120 }, --x,y,z,money
    [1] = { 2574.5144, 328.5554, 108.45, 230 },
    [2] = { 602.009521484375, 268.625, 103.472854614258, 340 },
    [3] = { 650.879333496094, 2768.23828125, 41.9326171875, 450 },
    [4] = { 2136.423828125, 4778.421875, 40.9705619812012, 600 },
...
function clear()
    ...
    if (DoesEntityExist(MISSION.trailer)) then
        DeleteVehicle(Citizen.PointerValueIntInitialized(MISSION.trailer))
        DeleteVehicle(MISSION.trailer)
        SetEntityAsNoLongerNeeded(MISSION.trailer)
    end
    if (DoesEntityExist(MISSION.truck)) then
        SetEntityAsNoLongerNeeded(MISSION.truck)
        SetVehicleDoorsLocked(MISSION.truck, 0)
...
function GUI.init()
    ...
    GUI.addButton(0, "Transport Articulé", GUI.optionMisson, 0.35, 0.35, 0.3, 0.05)
    ...
    GUI.addButton(1, "Mission 1 [ 120$ ]", GUI.mission, 0.35, 0.25, 0.3, 0.05)
    GUI.addButton(1, "Mission 2 [ 230$ ]", GUI.mission, 0.35, 0.30, 0.3, 0.05)
    GUI.addButton(1, "Mission 3 [ 340$ ]", GUI.mission, 0.35, 0.35, 0.3, 0.05)
    GUI.addButton(1, "Mission 4 [ 450$ ]", GUI.mission, 0.35, 0.40, 0.3, 0.05)
    GUI.addButton(1, "Mission 5 [ 600$ ]", GUI.mission, 0.35, 0.45, 0.3, 0.05)
...
function GUI.renderTitle()
    for id, settings in pairs(GUI.title) do
        ...
        --AddTextComponentString(settings["name"])
        GUI.renderBox(settings["xpos"], settings["ypos"], settings["xscale"], settings["yscale"],
...
function GUI.renderButtons(menu)
    for id, settings in pairs(GUI.button[menu]) do
        ...
        DrawText((settings["xpos"] + 0.001), (settings["ypos"] - 0.015))
        --AddTextComponentString(settings["name"])
        GUI.renderBox(settings["xpos"], settings["ypos"], settings["xscale"],
