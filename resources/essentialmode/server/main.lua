...
AddEventHandler('playerDropped', function()
    if (Users[source]) then
        ...
        --[[
			MySQL.Async.execute("UPDATE user_vehicle SET vehicle_state='Rentré' WHERE identifier = @identifier",
        ...
        if (command) then
            ...
            if (command.perm > 0) then
                if (Users[source].permission_level >= command.perm or Users[source].group:canTarget(command.group)) then
                    ...
                    TriggerEvent('Slog', '[ADMIN] id: '..source..' ,name: '..GetPlayerName(source)..', reason: '..'Admin ran command: '.. command_args[1])
                else
                    ...
                    TriggerEvent('Slog', '[WARNING] id: '..source..' ,name: '..GetPlayerName(source)..', reason: '..'Non admin attempted to run admin command: '.. command_args[1])
                end
            else
                command.cmd(source, command_args, Users[source])
                TriggerEvent("es:userCommandRan", source, command_args, Users[source])
                TriggerEvent('Slog', '[USER] id: '..source..' ,name: '..GetPlayerName(source)..', reason: '..'User ran command: '.. command_args[1])
            end
