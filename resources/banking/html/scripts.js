...
$(document).ready(function () {
    ...
    function openContainer(model) {
        if(model == "atm") $(".atm-container").css("display", "block");
        else $(".bank-container").css("display", "block");
        $("#cursor").css("display", "block");
    }

    function closeContainer() {
        $(".bank-container, .atm-container").css("display", "none");
    ...
    window.addEventListener('message', function (event) {
        ...
        if(item.openATM == true) {
            openContainer("atm");
            openMain();
        }
        if (item.openBank == false || item.openATM == false) {
            closeContainer();
            closeMain();
        }
        ...
        if (item.displayBalance === true) {
            $('.balance').delay(600).show();
        } else if (item.displayBalance === false) {
            $('.balance').hide();
        }
    });
    ...
    // Handle Form Submits
    $("#withdraw-form, #withdraw-atm-form").submit(function (e) {
        e.preventDefault();
        $.post('http://banking/withdrawSubmit', JSON.stringify({
            amount: $("#amount", (this)).val()
        }));
        $("#amount", (this)).prop('disabled', true)
        $("#submit", (this)).css('display', 'none')
        setTimeout(function () {
            $("#withdraw-form #amount, #withdraw-atm-form #amount").prop('disabled', false)
            $("#withdraw-form #submit, #withdraw-atm-form #submit").css('display', 'block')
        }, 1000)
        $("#amount", (this)).val('')
    });
