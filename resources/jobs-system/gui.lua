...
function Menu.renderButtons()
    ...
    SetTextDropShadow(0.1, 255, 0, 0, 165)
    SetTextEdge(0, 0, 0, 0, 0)
    SetTextEntry("STRING")
    AddTextComponentString("Métiers")
    DrawText((xoffset + 0.05), (yoffset - 0.075))
    Menu.renderBox(xoffset, 0.3, (yoffset - 0.05), 0.0625, 0, 0, 75, 180)
    ...
    for id, settings in pairs(Menu.GUI) do
        ...
        boxColor = { 0, 102, 204, 165 }
        if (settings["active"]) then
            boxColor = { 0, 0, 204, 195 }
        end
        SetTextFont(0)
        SetTextScale(0.0, 0.35)
        SetTextColour(255, 255, 255, 255)
        SetTextDropShadow(0.05, 0, 0, 0, 120)
        SetTextEdge(0, 0, 0, 0, 0)
        SetTextEntry("STRING")
        AddTextComponentString(settings["name"])
        DrawText(settings["xmin"] + 0.02, (settings["ymin"] - 0.0125))
